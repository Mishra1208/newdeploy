...
// Where the CSV lives (prefer /public to ship with the app)
...
function buildIndex(rows) {
  // Group by SUBJECT + CATALOGUE, merge multi-term offerings
  ...
  for (const r of rows) {
    ...
    // Prefer the first non-empty description/prereq/equivalent we see
    ...
    // Prefer a stronger title if present later
    if (safe(r.title) && safe(r.title).length > safe(entry.title).length) entry.title = safe(r.title);

    byCode.set(key, entry);
  }

  // Finalize (turn Sets into arrays and sort terms a little)
...
async function main() {
  ...
  // Also emit a quick lookup map: slug → entry (handy for the chatbot)
  const map = {};
