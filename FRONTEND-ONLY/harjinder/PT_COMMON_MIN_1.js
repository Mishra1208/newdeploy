...
function restoreComma(sVal) {
 
 var sTmpValue = sVal; if (sTmpValue.search('::::') >= 0) {
 sTmpValue = sTmpValue.replaceAll('::::', ','); if (sTmpValue.startsWith('"') && sTmpValue.endsWith('"')) {
 var nLngth = sTmpValue.length; sTmpValue = sTmpValue.substring(1, nLngth - 1); }
 }
 
 if (sTmpValue.search(',') >= 0) {
 if (sTmpValue.startsWith('"') && sTmpValue.endsWith('"')) {
 var nLngth = sTmpValue.length; sTmpValue = sTmpValue.substring(1, nLngth - 1); }
 }
 return sTmpValue;}
...
PT_Dialog.prototype = {
 ...
 role = " role='dialog' "; role += "aria-modal='true' "; if (modObj.bPopup || modObj.bHeader)
 role += "aria-labelledby='ptModTitle_" + this.idCount + "' "; }
 ...
 { 
 if (!bFluidModal && modObj.bStartup) 
 ...
 otb.style.right = modObj.nRight + 'px';   if (otb.getAttribute("role") && !otb.getAttribute("aria-label"))
 {
 var sTitle = getModalDialogTitle(); if (typeof sTitle != "undefined" && sTitle.length)
 otb.setAttribute("aria-label", sTitle); else if (currDoc)
 {
 const header = currDoc.getElementsByTagName("h1"); if (header && (header.length > 0) && header[0].firstElementChild)
 otb.setAttribute("aria-label", header[0].firstElementChild.innerText); }
 }
...
getModalDialogTitle: function(id) {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + id);  if (typeof modObj == "undefined" || !modObj)
 return ""; if (modObj.sTitle.length > 0)
...
function appendAttrValue(sElemId, sAttr, sVal) {
 if ((typeof sElemId === 'undefined') || (typeof sAttr === 'undefined') || (typeof sVal === 'undefined'))
 return; if (!sElemId || !sAttr || !sVal)
 return; var el = document.getElementById(sElemId); if (el) {
 var sAttrVal = el.getAttribute(sAttr); if ((typeof sAttrVal === 'undefined') || (!sAttrVal))
 sAttrVal = sVal; else
 sAttrVal = sAttrVal + ' ' + sVal; el.setAttribute(sAttr, sAttrVal); }
}

function prependAttrValue(sElemId, sAttr, sVal) {
 if ((typeof sElemId === 'undefined') || (typeof sAttr === 'undefined') || (typeof sVal === 'undefined'))
 return; if (!sElemId || !sAttr || !sVal)
 return; var el = document.getElementById(sElemId); if (el) {
 var sAttrVal = el.getAttribute(sAttr); if ((typeof sAttrVal === 'undefined') || (!sAttrVal))
 sAttrVal = sVal; else
 sAttrVal = sVal + ' ' + sAttrVal; el.setAttribute(sAttr, sAttrVal); }
}
