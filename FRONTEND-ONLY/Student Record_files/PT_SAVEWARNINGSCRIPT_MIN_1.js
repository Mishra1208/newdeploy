...
function ptPgltWarn(form, id, event) {
 ...
 var sYes = "Yes"; var sNo = "No";  var sChkBoxMsg = "Disable pagelet warning permanently. To enable again, go to your personalization page."; shtml = "<div id='msgTitle' style='display:none;'>" + msgTitle + "</div>";  shtml += "<div id='alertmsg' tabindex='0' role='presentation'><span class='popupText' id='alerttxt'>" + msg + "</span>"; shtml += "<div> <input type='checkbox' onclick='if(this.checked == true)" + evtYes + "' class='PSCHECKBOX' id='PERSN_PGLT_WARN' >"; shtml +="<label class='PSCHECKBOX' id='PERSN_PGLT_WARN_LBL' for='PERSN_PGLT_WARN'>" + sChkBoxMsg + "</label></div></div>";  shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' ><span><input type=button id='ALERTOK' class='PSPUSHBUTTONTBOK' value='" + sYes + "'";  shtml += " alt='" + sYes + "' title='" + sYes + "' onclick=\"pgltWarningOff();closeMsg(this);" + evtYes + evtPassForm + "\"/></span></a>";  shtml += "<a class='PSPUSHBUTTON Left' ><span><input type=button id='ALERTCANCEL' class='PSPUSHBUTTONTBOK' value='" + sNo + "'";  shtml += " alt='" + sNo + "' title='" + sNo + "' onclick='" + evtNo + "' /></span></a></div>";   addMsg(shtml, window, '');  playMsg();    var gpModTable = document.getElementById('alertmsg');  if (gpModTable) {
...
if ((changed==null) && top.frames)
 changed = checkAnyFrameChanged(top.frames);if ((changed==null) && isFModeLayout())
 changed = checkFrameChanged(window);if (isFModeLayout() && isSUOW()) {
 var agTarget = typeof document.getElementById("ICAGTarget") != 'undefined' && document.getElementById("ICAGTarget"); var agChanged = agTarget ? (typeof document.getElementById("ICUOW") != 'undefined' && document.getElementById("ICUOW") != null && document.getElementById("ICUOW").value != "N" ? true : false) : false; if (agChanged)
 return psConfirmFluid("Save Warning", "Do you want to save your changes?" , "YesNo", saveOKEvent, saveCancelEvent, "All the Changes in the Activity Guide will be lost. Click Yes to go back and save, No to discard your changes.", pWin, bDelay);}
var rtn = true;if (changed)
{
 if (typeof(url)!= 'undefined' && url != "") {
 var saveCancelEvent = ""; var saveOKEvent = ""; var bModal = typeof modalCloseUrl != 'undefined'; if (url.substr(0,10) == "javascript")
 {
 url = url.replace(/'/g, '\\"') + "|";  var actions = url.split("|"); url = ""; if (bModal) saveCancelEvent = 'if(oParentWin == null) oParentWin = window;'; saveCancelEvent += (bModal? 'oParentWin.' : '') + 'saveWarningEvent("'+url+'","'+target+'","'+actions[0]+'")';  if (actions[1] != "undefined" && actions[1] != "")
 if (bModal) saveOKEvent = 'if(oParentWin == null) oParentWin = window;'; saveOKEvent += (bModal? 'oParentWin.' : '') + 'saveWarningEvent("'+url+'","'+target+'","'+actions[1]+'")'; }
...
{
 if(top.document.documentElement.className.includes("pt_rte_updated") && !(document.getElementById("PT_AGSTARTPAGE_NUI")))
 { 
 return true;  }
 else
 {
 ...
 return (obj.checked != obj.defaultChecked) && !ignoreChg(obj, objFrame);else if (obj.type == "select-one" && obj.selectedIndex > -1)
{
 ...
 return false; else if((obj.id.match(/[A-Za-z0-9_].*\$hpage\$[0-9]*/) && obj.id.match(/[A-Za-z0-9_].*\$hpage\$[0-9]*/).length > 0)
 || (obj.id.match(/[A-Za-z0-9_].*\$fpage\$[0-9]*/) && obj.id.match(/[A-Za-z0-9_].*\$fpage\$[0-9]*/).length > 0)) 
 return false; else
...
else if (obj.type == "select-multiple")
{
 ...
 return false;   const currenttime = new Date(obj.value); const oldtime = new Date(obj.defaultValue); if(currenttime.getTime() != oldtime.getTime())
 {
 return true; }
 else 
 return false; }
...
function setupTimeout2(nTimeoutMilliseconds, nWarningTimeoutMilliseconds)
{
 UpdateDeviceCookie(); if (typeof (totalTimeoutMilliseconds) != 'undefined' && totalTimeoutMilliseconds <= 2073200000) {
 var bReset = false; if (typeof nTimeoutMilliseconds == "undefined" || typeof nWarningTimeoutMilliseconds == "undefined") {
 nTimeoutMilliseconds = totalTimeoutMilliseconds; nWarningTimeoutMilliseconds = warningTimeoutMilliseconds; }
 else
 bReset = true; if (!isSessionLoggedout(false)) {
 clearTimers(); if (!bAutoSave) {
 timeoutID = window.setTimeout('displayTimeoutMsg2()', nTimeoutMilliseconds); if (isTopFModeLayout())
 timeoutWarningID = window.setTimeout('displayTimeoutWarningFluid()', nWarningTimeoutMilliseconds); else
 timeoutWarningID = window.setTimeout('displayTimeoutWarningMsg2()', nWarningTimeoutMilliseconds); }
 }

 if (!bReset && window != top && isTopFModeLayout()) {
 top.setupTimeout2(nTimeoutMilliseconds + 10, nWarningTimeoutMilliseconds + 10); for (j = 0; j < top.frames.length; j++) {
 var oWin = top.frames[j]; try {
 if (typeof oWin != "undefined" && oWin != window)
 oWin.setupTimeout2(nTimeoutMilliseconds + 10, nWarningTimeoutMilliseconds + 10); }
 catch (e) {
 }
 }
 }
...
timeoutWinOption = "toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=0,resizable=0,width=470,height=170";timeoutWin =window.open( timeoutWarningPageURL, "timeoutWarning", timeoutWinOption, true);timeoutWin.focus();if ((typeof(isAutoSaveEnabled) == "function") && isAutoSaveEnabled()) {
...
var allcookies;allcookies = document.cookie;if (allcookies == "")
 return "-1";var start = allcookies.indexOf(cookiename + '=');if (start == -1) return "-1"; start += cookiename.length + 1; var end = allcookies.indexOf(';', start);if (end == -1) end = allcookies.length;var cookieval = allcookies.substring(start, end);cookieval = cookieval.replace("\"","");var a = cookieval.split(' ');var winhref;try { 
...
function psConfirmSW(saveOKEvent, saveCancelEvent, pWin) {
 if (isFModeLayout() && isSUOW) {
 var agTarget = typeof document.getElementById("ICAGTarget") != 'undefined' && document.getElementById("ICAGTarget"); var agChanged = agTarget ? (typeof document.getElementById("ICUOW") != 'undefined' && document.getElementById("ICUOW") != null && document.getElementById("ICUOW").value != "N" ? true : false) : false; if (agChanged)
 return psConfirmFluid("Save Warning", "Do you want to save your changes?" , "YesNo", saveOKEvent, saveCancelEvent, "All the Changes in the Activity Guide will be lost. Click Yes to go back and save, No to discard your changes.", pWin); }

 if (!isFModeLayout() && !isTopFModeLayout()) {
 if (typeof isNewSaveWarn !== "undefined" && isNewSaveWarn) {
 return psConfirmClassic("Save Warning", "Do you want to save your changes? Click Yes to go back and save, or No to discard your changes.", "YesNo", saveOKEvent, saveCancelEvent, pWin); } else {
 return psConfirmClassic("Save Warning", "You have unsaved data on this page. Click OK to go back and save, or Cancel to continue.", "OKCancel", saveCancelEvent, saveOKEvent, pWin); }
 }
 else {
...
function psConfirm2(msgTitle, msg, msgType, event1, event2, pWin, bDelay) {
 ...
 var onClickStr = ""; if(isClassicPlus())
 {
 onClickStr = " onclick='javascript:if(event.target.id == this.firstChild.firstChild.id) { return true; } else return((this.firstChild &amp;&amp; this.firstChild.firstChild) != null ? this.firstChild.firstChild.click() : false);'" ; }

 if (typeof modalCloseUrl != 'undefined' && modalCloseUrl) {
 var sOK = "OK",
 sCancel = "Cancel",
 sYes = "Yes",
 sNo = "No"; shtml = "<div id='ptpopup' class='popupText' role='alertdialog' aria-labelledby='msgTitle' aria-describedby='alertmsg'>"
 shtml += "<div id='msgTitle' style='display:none;'>" + msgTitle + "</div>"; shtml += "<div id='alertmsg' class='popupText'>" + msg + "</div>"; if (msgType == "OKCancel") {
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style = 'border:0;'" + onClickStr + "><span><input type=button id='#ALERTOK' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sOK + "' tabindex='0' alt='" + sOK + "' title='" + sOK + "' onclick='" + event2 + ";closeMsg(this);' /></span></a>"); shtml += ("<a class='PSPUSHBUTTON Left' style = 'border:0;'" + onClickStr + "><span><input type=button id='#ALERTCANCEL' name='#ALERTCANCEL' class='PSPUSHBUTTONTBOK' value='"); shtml += (sCancel + "' tabindex='0' alt='" + sCancel + "' title='" + sCancel + "' onclick='" + event1 + ";closeMsg(this);' /></span></a></div>"); } else if (msgType === "YesNo") {
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style='border:0;'" + onClickStr + "><span><input type='button' id='#ALERTYES' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sYes + "' tabindex='0' alt='" + sYes + "' title='" + sYes + "' onclick='" + event1 + ";closeMsg(this);' /></span></a>"); shtml += ("<a class='PSPUSHBUTTON Left' style='border:0;'" + onClickStr + "><span><input type='button' id='#ALERTNO' name='#ALERTNO' class='PSPUSHBUTTONTBOK' value='"); shtml += (sNo + "' tabindex='0' alt='" + sNo + "' title='" + sNo + "' onclick='" + event2 + ";closeMsg(this);' /></span></a>"); } else if (msgType == "YesNoCancel") {
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style = 'border:0;'" + onClickStr + "><span><input type=button id='#ALERTYES' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sYes + "' tabindex='0' alt='" + sOK + "' title='" + sYes + "' onclick='" + event1 + ";closeMsg(this);' /></span></a>"); shtml += ("<a class='PSPUSHBUTTON Left' style = 'border:0;'" + onClickStr + "><span><input type=button id='#ALERTNO' name='#ALERTNO' class='PSPUSHBUTTONTBOK' value='"); shtml += (sNo + "' tabindex='0' alt='" + sCancel + "' title='" + sNo + "' onclick='" + event2 + ";closeMsg(this);' /></span></a>"); shtml += ("<a class='PSPUSHBUTTON Left' style = 'border:0;'" + onClickStr + "><span><input type=button id='#ALERTCANCEL' name='#ALERTCANCEL' class='PSPUSHBUTTONTBOK' value='"); shtml += (sCancel + "' tabindex='0' alt='" + sCancel + "' title='" + sCancel + "' onclick='closeMsg(this);' /></span></a></div>"); } else if (msgType == "OKANDEVT") { 
 
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style = 'border:0;'" + onClickStr + "><span><input type=button id='#ALERTOK' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sOK + "' tabindex='0' alt='" + sOK + "' title='" + sOK + "' onclick='closeMsg(this);" + event1 + "'/></span></a></div>");  } else {
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style = 'border:0;'" + onClickStr + "><span><input type=button id='#ALERTOK' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sOK + "' tabindex='0' alt='" + sOK + "' title='" + sOK + "' onclick='closeMsg(this);' /></span></a></div>"); }
 shtml += "</div>"; var options = 'closeUrl: ' + modalCloseUrl + ';closeAlt:' + modalCloseAlt + ';resizeUrl: ' + modalResizeUrl + ';resizeAlt:' + modalResizeAlt + ';moveAlt:' + modalMoveAlt + ';'; addMsg(shtml, pWin, options); if (!bDelay) { playMsg(); }
...
function psConfirmClassic(msgTitle, msg, msgType, event1, event2, pWin) {
 if (typeof pWin == 'undefined' || pWin == null) { pWin = window; }

 var mCloseUrl = mCloseAlt = mResizeUrl = mResizeAlt = mMoveAlt = ""; if (typeof modalCloseUrl != 'undefined' && modalCloseUrl) {
 mCloseUrl = modalCloseUrl; mCloseAlt = modalCloseAlt; mResizeUrl = modalResizeUrl; mResizeAlt = modalResizeAlt; mMoveAlt = modalMoveAlt;  } else {
 var cWindow = document.getElementById("ptifrmtgtframe").contentWindow; if (typeof cWindow == 'undefined' || !cWindow) { return; }
 mCloseUrl = cWindow.modalCloseUrl; mCloseAlt = cWindow.modalCloseAlt; mResizeUrl = cWindow.modalResizeUrl; mResizeAlt = cWindow.modalResizeAlt; mMoveAlt = cWindow.modalMoveAlt; }

 var sOK = "OK",
 sCancel = "Cancel",
 sYes = "Yes",
 sNo = "No"; var shtml = "<div id='msgTitle' style='display:none;'>" + msgTitle + "</div>"; shtml += "<div id='alertmsg'><span class='popupText'>" + msg + "</span></div>"; if (msgType == "OKCancel") {
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style = 'border:0;'><span><input type=button id='#ALERTOK' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sOK + "' tabindex='0' alt='" + sOK + "' title='" + sOK + "' onclick='" + event2 + ";closeMsg(this);' /></span></a>"); shtml += ("<a class='PSPUSHBUTTON Left' style = 'border:0;'><span><input type=button id='#ALERTCANCEL' name='#ALERTCANCEL' class='PSPUSHBUTTONTBOK' value='"); shtml += (sCancel + "' tabindex='0' alt='" + sCancel + "' title='" + sCancel + "' onclick='closeMsg(this);" + event1 + ";' /></span></a></div>"); } else if (msgType == "OK") { 
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style = 'border:0;'><span><input type=button id='#ALERTOK' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sOK + "' tabindex='0' alt='" + sOK + "' title='" + sOK + "' onclick='closeMsg(this);' /></span></a></div>"); } else {
 shtml += "<div id='alertbutton'><a class='PSPUSHBUTTON Left' style='border:0;'><span><input type='button' id='#ALERTYES' name='#ALERTOK' class='PSPUSHBUTTONTBOK' value='"; shtml += (sYes + "' tabindex='0' alt='" + sYes + "' title='" + sYes + "' onclick='" + event1 + ";closeMsg(this);' /></span></a>"); shtml += ("<a class='PSPUSHBUTTON Left' style='border:0;'><span><input type='button' id='#ALERTNO' name='#ALERTNO' class='PSPUSHBUTTONTBOK' value='"); shtml += (sNo + "' tabindex='0' alt='" + sNo + "' title='" + sNo + "' onclick='closeMsg(this);" + event2 + ";' /></span></a>"); }

 var options = 'closeUrl: ' + mCloseUrl + ';closeAlt:' + mCloseAlt + ';resizeUrl: ' + mResizeUrl + ';resizeAlt:' + mResizeAlt + ';moveAlt:' + mMoveAlt + ';'; addMsg(shtml, pWin, options); playMsg();}
...
function psConfirmFluid(msgTitle, msg, msgType, event1, event2, longMsg, pWin, bDelay) {
 ...
 shtml += "<div id='#ICOK' class='ps_box-button psc_ok'><span id='#ICOK$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTOK' role='button'"; shtml += " onclick='" + event2 + ";closeLastModal(this);' href='javascript:void(0);'/><span class='ps-text'>" + sOK + "</span></a></span></div>"; shtml += "<div id='#ICCancel' class='ps_box-button psc_cancel'><span id='#ICCancel$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTCANCEL' role='button'"; shtml += " onclick='" + event1 + ";closeLastModal();' href='javascript:void(0);'/><span class='ps-text'>" + sCancel + "</span></a></span></div>"; } else if (msgType === "YesNo") {
 shtml += "<div id='#ICYes' class='ps_box-button psc_yes'><span id='#ICYes$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTYES' role='button'"; shtml += " onclick='closeLastModal();" + event1 + ";' href='javascript:void(0);'/><span class='ps-text'>" + sYes + "</span></a></span></div>"; shtml += "<div id='#ICNO' class='ps_box-button psc_no'><span id='#ICNO$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTNO' role='button'"; shtml += " onclick='closeLastModal();" + event2 + ";' href='javascript:void(0);'/><span class='ps-text'>" + sNo + "</span></a></span></div>"; } else if (msgType == "YesNoCancel") {

 shtml += "<div id='#ICYes' class='ps_box-button psc_yes'><span id='#ICYes$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTYES' role='button'"; shtml += " onclick='" + event1 + ";closeLastModal();' href='javascript:void(0);'/><span class='ps-text'>" + sYes + "</span></a></span></div>"; shtml += "<div id='#ICNO' class='ps_box-button psc_no'><span id='#ICNO$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTNO' role='button'"; shtml += " onclick='" + event2 + ";closeLastModal();' href='javascript:void(0);'/><span class='ps-text'>" + sNo + "</span></a></span></div>"; shtml += "<div id='#ICCancel' class='ps_box-button psc_cancel'><span id='#ICCancel$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTCANCEL' role='button'"; shtml += " onclick='closeLastModal();' href='javascript:void(0);'/><span class='ps-text'>" + sCancel + "</span></a></span></div>"; } else if (msgType == "OKANDEVT") {
 
 shtml += "<div id='#ICOK' class='ps_box-button psc_ok'><span id='#ICOK$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTOK' role='button'"; shtml += " onclick='closeLastModal();" + event1 +"' href='javascript:void(0);'/><span class='ps-text'>" + sOK + "</span></a></span></div>"; } else {
 shtml += "<div id='#ICOK' class='ps_box-button psc_ok'><span id='#ICOK$span' class='ps-button-wrapper'><a class='ps-button' id='#ALERTOK' role='button'"; shtml += " onclick='closeLastModal();' href='javascript:void(0);'/><span class='ps-text'>" + sOK + "</span></a></span></div>"; }
...
function pingServer(url) {
 if (!isTopFModeLayout()) return; if (typeof top.workers == "undefined" || workers.length == 0) return; var pingUrl = getUrlHost(url) + "/psc/" + trimWindowsNumber0(getPSHome(url)) + '/?cmd=ping'; top.doGrouplet(pingUrl);}

function displayTimeoutWarningFluid() 
{
 if (isLastTimeoutMsgModal()) return; var nhowLong = totalTimeoutMilliseconds - warningTimeoutMilliseconds - 2000; var nTimeMins = Math.round(totalTimeoutMilliseconds / 60000); var sOK = "OK"; var timeoutWarningMsg = "Your session is about to expire."; var sLongMsg = "As a security precaution, sessions end after"; sLongMsg += " "+nTimeMins+" "; sLongMsg += "minutes of inactivity. <br/><br/>Click OK to continue your current session."; var shtml = "<div id='msgcontainer' class='ps_box-msgcontainer' role='alertdialog' aria-describedby='alertmsg'><div id='alertmsg' class='ps_box-msgalert'><div id='shortmsg' class='ps_box-msgshort'>"; shtml += timeoutWarningMsg; shtml += "</div><div id='msgnum' class='ps_box-msgnum'></div><div id='othermsg' class='ps_box-msgother'></div><div id='longmsg' class='ps_box-msglong'>"+sLongMsg+"</div></div>"; shtml += "<div id='buttoncontainer' class='ps_box-msgactions'><div class='ps_box-button' id='div#ICOK'><span id='#ICOK$span' class='ps-button-wrapper'><a id='#ICOK' class='ps-button' role='button' href='javascript:pingServer(\""+EscapeJSString(window.location.href)+"\");setupTimeout2();closeLastModal();'>"; shtml += "<span class='ps-text'>"+ sOK+"</span></a></span></div></div></div>"; addMsg(shtml, getLastModalWin(),"bMsg@1;bTimeoutMsg@1;bAutoClose@0;"); playMsg();}
...
function setDeviceCookie(name, value, exdays, path, domain, secure) {
...
function getCookie(cname) {
 ...
 var value = c.substring(name.length+c.indexOf(name), c.length); if (value == '""')
 return ""; else
 return value; }
 }
 return "";}
...
function updatePTCookie(jsonString, cookieName) {
 var sCookieValue = jsonString.replace(/"/g, ""); sCookieValue = sCookieValue.replace(/{/g, ""); sCookieValue = sCookieValue.replace(/}/g, ""); sCookieValue = sCookieValue.replace(/ /g, ""); sCookieValue = sCookieValue.replace(/,/g, " "); var scheme = window.location.href.substr(0, 5); var secure = (scheme == "https") ? true : null; setDeviceCookie(cookieName, sCookieValue, 1000, "/", document.domain, secure);}

function getFormFactorSize(ff)
{
 var formfactor = "3"; if(typeof(ff) != "undefined" && ff != null && ff)
 {
 formfactor = ff; } 
 else
 formfactor = document.getElementById("ptformfactor") ? document.getElementById("ptformfactor").value : getFormFactorFromURL(); var devWidth, devHeight; if (formfactor == "0") {
...
function applyFormFactor() { 
 var devWidth = getFormFactorSize().width; var devHeight = getFormFactorSize().height; var objWrap = document.getElementById("pswrapper"); var url = window.location.href; var regexPTF = new RegExp("[?&]" + "ptformfactor" + "=([^&]*)", "i"); var pattPTF = regexPTF.exec(url); if ((typeof pattPTF != "undefined") && (pattPTF.length > 0))
...
function ptDeviceFeatures() {
 ...
 try {
 var testHC = document.createElement('div'); testHC.style.color = 'rgb(200, 100, 50)'; document.body.appendChild(testHC); var color = document.defaultView.getComputedStyle(testHC, null).color; document.body.removeChild(testHC); return color !== 'rgb(200, 100, 50)' ? 1 : 0; } catch (e) {
 return 0; }
 };};