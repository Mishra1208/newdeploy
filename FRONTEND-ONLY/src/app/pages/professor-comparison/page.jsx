        ...
        try {
            ...
            if (data.success && data.data) {
                ...
                return {
                    ...
                    avgDifficulty: p.difficulty || 0,
                    wouldTakeAgainPercent: p.takesAgain || 0,
                ...
                <AnimatePresence>
                    {mode === "single" && p1 && !loading && (
                        ...
                        >
                                ...
                                >
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <div className={`flex items-center gap-2 text-sm ${textMuted}`}>
                                            <Award size={16} />
                                            <span>{p1.schoolName || p1.school?.name || "Unknown School"}</span>
                                        </div>
                                    </div>
                                </motion.div>
                            ...
                            {/* [NEW] Teaching History Section */}
                            <div className={`md:col-span-2 ${cardClass} border rounded-3xl p-8 relative overflow-hidden h-full`}>
                                <h3 className={`${textMuted} font-semibold mb-6 flex items-center gap-2 uppercase tracking-wide text-xs`}>
                                    <BookOpen size={16} /> Course Teaching History
                                </h3>
                                <div className="flex flex-wrap gap-2">
                                    {p1.courses && p1.courses.length > 0 ? (
                                        p1.courses.map((course, idx) => (
                                            <motion.div
                                                key={idx}
                                                initial={{ opacity: 0, scale: 0.8 }}
                                                animate={{ opacity: 1, scale: 1 }}
                                                transition={{ delay: idx * 0.05 }}
                                                className={`px-4 py-2 rounded-xl text-sm font-bold border ${isDark
                                        ...
                                        <div className={`text-sm ${textMuted} italic`}>No historical course data available for this professor.</div>
                                    )}
                                </div>
                                <p className={`mt-6 text-[10px] ${textMuted} uppercase tracking-widest font-bold opacity-60`}>
                                    * Data derived from student review history
                                </p>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {mode === "compare" && p1 && p2 && !loading && (
                        ...
                        >
                            ...
                            <div className={`grid grid-cols-3 gap-y-6 gap-x-8 mt-12 border-t pt-12 items-center text-center ${isDark ? "border-white/5" : "border-slate-100"}`}>
                                ...
                                {/* Teaching History Comparison */}
                                <div className="md:col-span-3 border-t mt-12 pt-12 text-left">
                                    <h3 className={`${textMuted} font-semibold mb-6 flex items-center justify-center gap-2 uppercase tracking-wide text-xs`}>
                                        <BookOpen size={16} /> Course Teaching History Comparison
                                    </h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                        <div className="space-y-4">
                                            <div className={`text-sm font-bold ${isDark ? "text-white" : "text-slate-900"} mb-2`}>{p1.lastName}'s Courses</div>
                                            <div className="flex flex-wrap gap-2">
                                                {p1.courses && p1.courses.length > 0 ? (
                                                    p1.courses.map((c, i) => (
                                                        <span key={i} className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${isDark ? "bg-blue-500/10 border-blue-500/20 text-blue-400" : "bg-blue-50 border-blue-100 text-blue-700"}`}>
                                                            {c}
                                                        </span>
                                                    ))
                                        ...
                                        <div className="space-y-4">
                                            <div className={`text-sm font-bold ${isDark ? "text-white" : "text-slate-900"} mb-2`}>{p2.lastName}'s Courses</div>
                                            <div className="flex flex-wrap gap-2">
                                                {p2.courses && p2.courses.length > 0 ? (
                                                    p2.courses.map((c, i) => (
                                                        <span key={i} className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${isDark ? "bg-amber-500/10 border-amber-500/20 text-amber-500" : "bg-amber-50 border-amber-100 text-amber-700"}`}>
                                                            {c}
                                                        </span>
                                                    ))
                                                ) : <span className={`text-xs ${textMuted} italic`}>No data</span>}
