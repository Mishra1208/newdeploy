...
import { JMSB_PROGRAMS } from '../../../../utils/degreeEngine/data/programs/jmsb/index.js';
...
const categoryColors = {
  // Main Program Cards (Step 0)
  'Accounting': 'text-rose-900 dark:text-rose-300 bg-rose-100 dark:bg-rose-900/40 border-rose-500/30',
  'Economics & Finance': 'text-emerald-900 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-900/40 border-emerald-500/30',
  'Marketing & IB': 'text-violet-900 dark:text-violet-300 bg-violet-100 dark:bg-violet-900/40 border-violet-500/30',
  'Tech & Supply Chain': 'text-sky-900 dark:text-sky-300 bg-sky-100 dark:bg-sky-900/40 border-sky-500/30',
  'Management & HR': 'text-amber-900 dark:text-amber-300 bg-amber-100 dark:bg-amber-900/40 border-amber-500/30',
  'Minors & Certificates': 'text-slate-800 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/40 border-slate-500/30',

  // Inner Degree Pathfinder Course Blocks
  'JMSB Core 200-Level': 'text-rose-800 dark:text-rose-200 bg-rose-50 dark:bg-rose-900/20 border-rose-300/30',
  'JMSB Core 300 & 400-Level': 'text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-900/20 border-amber-300/30',
  'Accountancy Major Core': 'text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800/40 border-slate-500/30',
  'Accountancy Electives': 'text-blue-800 dark:text-blue-200 bg-blue-50 dark:bg-blue-900/20 border-blue-300/30'
};
...
const categoryIcons = {
  ...
  'Minors & Certificates': <svg className="w-32 h-32 text-slate-500/10 dark:text-slate-500/20 dark:drop-shadow-[0_0_15px_rgba(100,116,139,0.3)] group-hover:scale-110 transition-transform duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
};
...
export default function FreshDegreeTracker() {
  ...
  const [selectedProgramId, setSelectedProgramId] = useState('accountancy');
  const [selectedOptionId, setSelectedOptionId] = useState(null);
  const [selectedMinorId, setSelectedMinorId] = useState('none');
  ...
  useEffect(() => {
    async function fetchPlan() {
      if (isLoaded && user) {
        const saved = await getDegreePlan(user.id, 'jmsb');
        if (saved) {
  ...
  const saveToProfile = useCallback(async () => {
    ...
    await saveDegreePlan(user.id, 'jmsb', stateToSave);
  ...
  }, [user, selectedProgramId, selectedMinorId, completedCourses, electiveChoices, delayedCourses, skippedSemesters, manualAdds, includeMathProfile, startTerm, startYear]);
  ...
  const handleConfirmForget = async () => {
    await deleteDegreePlan(user.id, 'jmsb');
    ...
    Object.values(PROGRAMS).forEach(prog => {
      if (prog.options) {
        prog.options.forEach(opt => {
          if (opt.id.includes('minor') || opt.name.includes('Minor')) {
            minors.push({ ...opt, parentId: prog.id, parentCategory: prog.category, globalId: `${prog.id}-${opt.id}` });
          }
        });
      }
    });
  ...
  const curriculum = useMemo(() => {
    if (!currentOption) return selectedProgram.data;
    
    // Deep clone the primary curriculum
    const baseCurriculum = JSON.parse(JSON.stringify(currentOption.data));
    
    if (selectedMinorId !== 'none') {
      // Find the minor data
      const minorObj = allMinors.find(m => m.globalId === selectedMinorId);
      if (minorObj && minorObj.data) {
        // 1. Delete 12 credits of General Electives
        let deletedCredits = 0;
        baseCurriculum.requirements = baseCurriculum.requirements.filter(req => {
          if (req.category === "General Electives" && deletedCredits < 12.0) {
            deletedCredits += req.credits;
            return false; // Remove it
          }
          return true;
        });
        
        // 2. Inject the Minor's requirements
        if (minorObj.data.requirements) {
          minorObj.data.requirements.forEach(req => {
            baseCurriculum.requirements.push(JSON.parse(JSON.stringify(req)));
          });
        }
        
        // 3. Merge courses dictionary if any custom ones exist
        if (minorObj.data.courses) {
           baseCurriculum.courses = { ...baseCurriculum.courses, ...minorObj.data.courses };
        }
      }
    }
    
    return baseCurriculum;
  }, [currentOption, selectedProgram, selectedMinorId, allMinors]);
                  ...
                  <>
                    <div className="bg-white dark:bg-white/[0.03] backdrop-blur-md p-6 rounded-2xl border border-slate-200 dark:border-white/[0.08] shadow-sm flex flex-col md:flex-row gap-6 justify-between items-center">
                      <div className="flex-1">
                        <h4 className="text-lg font-bold text-slate-800 dark:text-white/90">Primary Degree Path</h4>
                        <p className="text-sm text-slate-500 dark:text-white/60 font-medium">Select your major or honours option.</p>
                      </div>
                      <div className="flex gap-3 w-full md:w-auto">
                        ...
                        >
                          {selectedProgram.options
                            .filter(opt => !opt.id.includes('minor') && !opt.name.includes('Minor'))
                            .map(opt => (
                    ...
                    <div className="bg-white dark:bg-white/[0.03] backdrop-blur-md p-6 rounded-2xl border border-slate-200 dark:border-white/[0.08] shadow-sm flex flex-col md:flex-row gap-6 justify-between items-center">
                      <div className="flex-1">
                        <h4 className="text-lg font-bold text-slate-800 dark:text-white/90">Add a Minor <span className="text-sm font-bold text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded ml-2">Optional</span></h4>
                        <p className="text-sm text-slate-500 dark:text-white/60 font-medium">A 12-credit minor replaces your free General Electives.</p>
                      </div>
                      <div className="flex gap-3 w-full md:w-auto">
                        <select 
                        ...
                        >
                          ...
                          {allMinors.map(opt => (
                            <option 
                              key={opt.globalId} 
                              value={opt.globalId} 
                              disabled={opt.parentId === selectedProgramId}
                            >
                              {opt.name} {opt.parentId === selectedProgramId ? '(Ineligible)' : ''}
                            </option>
                          ))}
                        </select>
                      </div>
                    ...
                    <div className="relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-3xl p-10 rounded-[2.4rem] border border-white/20 dark:border-white/5 shadow-[0_32px_64px_-16px_rgba(145,35,56,0.15)] flex flex-col md:flex-row justify-between items-center gap-8 overflow-hidden">
                      ...
                      <div className="relative z-10 flex flex-col sm:flex-row items-center gap-4 mt-6 md:mt-0">
                        ...
                        <button 
                          ...
                          className="w-full sm:w-auto p-4 rounded-2xl bg-slate-100/50 dark:bg-white/5 text-slate-500 dark:text-white/50 hover:bg-rose-100 dark:hover:bg-rose-500/10 hover:text-rose-600 dark:hover:text-rose-400 transition-colors flex items-center justify-center group"
                          title="Forget saved plan"
                        >
                          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
