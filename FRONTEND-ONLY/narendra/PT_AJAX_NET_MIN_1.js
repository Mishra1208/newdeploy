...
if (this.bCallBack || !form || !form.ICAction || !form.ICResubmit || this.id.indexOf('Resubmit') != -1) return true;try {
 ...
 var sScript = "var chgFldArr = this.doc.chgFldArr_" + this.form.name + ";"; eval(sScript); if (this.bTypeAhead && (isFModeLayout() || chgFldArr.length == 1 && this.TAFieldID == chgFldArr[0]))
 { 
 ...
 var linkList = document.querySelectorAll("a[ps_state]"); if (linkList)
 {
 for (var j = 0; j < linkList.length; j++)
 {
 var eObj = linkList[j]; var eId = eObj.id; if (typeof eId != 'undefined' && eId)
 {
 sTmp += ptCommonObj2.getNV(eObj); sTmpModal2 += ptCommonObj2.getNV(eObj); }
 }
 }
 ...
 var rtefields = document.getElementsByClassName('cke5editor'); if(rtefields.length > 0)
 {
 for(f = 0 ; f < rtefields.length ; f++)
 {
 var rteckefiveScript = "addchg_"+this.form.name+"(rtefields[f]);"; eval(rteckefiveScript)
 }
 }

 
 var rtefieldsforckefour = document.getElementsByClassName('ps-ckeditor-data'); if(rtefieldsforckefour.length > 0)
 {
 for(var k = 0 ; k < rtefieldsforckefour.length ; k++)
 {
 var rteckefourScript = "addchg_"+this.form.name+"(rtefieldsforckefour[k]);"; eval(rteckefourScript); }
 }
...
reduceScrollareaHeight:function(id, data, obj)
 ...
 if(sCleanData.indexOf("CKEDITOR.replace") !== -1)
 this.processCKEditorFourScriptFmode(id, sCleanData); var idGridMod = getPTDialog().getGridSortModalId(); if (idGridMod.length != 0) {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + idGridMod); this.sortid = modObj.sortid; autoCloseGridSort(); }
 }
 else if(isFModeLayout() && classValue.indexOf("psc_has_ckeditor") !== -1)
 {
 
 this.processCKEditorFourScriptFmode(id, sCleanData); }
 else if(isFModeLayout() && sCleanData.indexOf("CKEDITOR.replace") !== -1)
 {
...
doPopupAction: function (id, fieldData)
{
 ...
 mObj.innerHTML = fieldData.firstChild.data; mObj.setAttribute('options', sOptions);  var optionsParts = sOptions.split(";"), prntEleId = null; for (var i =0; i < optionsParts.length; i++) {
 ...
 {
 var value1 = sOptions.indexOf("$ctxmenu$img"); var moptions_mod = sOptions.substring(0,value1); moptions_mod = moptions_mod + sOptions.substring(value1+12,sOptions.length); sOptions = moptions_mod; }
 }
 doCloseLocalModals(); setLocalModal(true); addDivPopup(mObj, this.win, sOptions, "", bFrame); return true;},
...
closeModal: function(ID){
 ...
 if ((typeof ID != 'undefined' && ID != null) || (typeof window.modWin != 'undefined' && window.modWin != null)) {
...
var obScript = null;var oHead = document.getElementsByTagName("head")[0];if (isFModeLayout() && src) {
 ...
 if (bMDTarget)
 obScript.setAttribute("pttarget", "js");  if(dscript.indexOf("CKEDITOR.replace") !== -1)
 oHead.removeChild(obScript); else
 return; }
 else 
...
processCKEditorFourScriptFmode:function(id,data)
{
var sData = new String(data);var myExp2= new RegExp("</script","gi");sData = sData.replace(/<script/gi,"%script");sData = sData.replace(myExp2,"%/script");if (sData.indexOf("%script")==-1) return data;var nLen = (net2.ENDSCRIPT).length; var pos=sData.indexOf("%script");if (pos != -1) this.bScript=true;var pos2;while(pos>-1){
 pos2=sData.indexOf(net2.ENDSCRIPT,pos);  var sTmp = sData.substring(pos,pos2); pos3 = sTmp.indexOf(">"); var sTmp0 = sTmp.substring(0,pos3)
...
function MoveCke5ScriptsToHeader()
{
 DestroyCke5Fields(); var cke5editorlist = document.getElementsByTagName("CKEDITOR5SCRIPTTAG");  if(cke5editorlist.length > 0)
 {
 for(var e = 0; e < cke5editorlist.length; e++)
 {
 
 var l = "ckeditor5Script(".length ; var fldId = cke5editorlist[e].innerText.substring(l + 1, cke5editorlist[e].innerText.length - 3);  var tmp = document.getElementById(fldId + "_SCRIPT"); if(tmp === null)
 {
 var oHead = document.getElementsByTagName("head")[0]; var obScript = document.createElement("script"); obScript.setAttribute("type","text/javascript"); obScript.setAttribute("id",fldId + "_SCRIPT"); obScript.innerHTML = cke5editorlist[e].innerText; oHead.appendChild(obScript);  }

 }
 }
}

function DestroyCke5Fields()
{
 var cke5editorlist = document.getElementsByTagName("CKEDITOR5SCRIPTTAG");  if(cke5editorlist.length > 0)
 {
 var cke5EestroyEvent = new CustomEvent("destroy_cke5_instance"); for(var e = 0; e < cke5editorlist.length; e++)
 {
 
 var l = "ckeditor5Script(".length ; var fldId = cke5editorlist[e].innerText.substring(l + 1, cke5editorlist[e].innerText.length - 3); var rteID = fldId.split('\''); var tmp = document.getElementById(fldId); var tmp_scripttag = document.getElementById(fldId + "_SCRIPT"); if(tmp !== null)
 tmp.dispatchEvent(cke5EestroyEvent); if(tmp_scripttag !== null)
 {
 
 var txtareaTag = document.getElementById(rteID[0]);  var ckeditorTag = txtareaTag.nextSibling; ckeditorTag.remove(); tmp_scripttag.remove(); }
 }
 } 
}

function PTRTEFillcacheCke5(URI,UrlID,UrlRepository,EditorField,DisplayOnly)
{
 var EditorString =""; var psSiteName=""; var DatafromXML=""; var EditorId = EditorField ;  var temp=""; var newdiv = document.createElement('div'); var divIdName = 'PTRTE_'+EditorId; var dblocation=UrlRepository.split("record://");  var EditorStringObj = document.getElementById(EditorId); if ((typeof(EditorStringObj) != 'undefined') && (EditorStringObj != null)) {
 if (DisplayOnly == 1)
 ...
 {
 var img = images[i]; if(img.src.startsWith("data:image"))
 {
  
  continue; }

 if(images[i].id.includes("###"))
 { 
 
 var tempname= images[i].id;  var srcValueToUpdate = images[i].outerHTML.split("\"")[1]; var rtetextareaValue = EditorStringObj.value ; var fileName = tempname.split("###")[1]; var count = (rtetextareaValue.split(fileName).length - 1);   if(count == 1)
 {
 var updatedRteValue = rtetextareaValue.replace(new RegExp(srcValueToUpdate, 'g'), srcValueToUpdate+"###"+fileName);  var psSiteNameForCKEv4SavedImage = images[i].getAttribute("src").split("/")[2]; if(psSiteNameForCKEv4SavedImage && psSiteNameForCKEv4SavedImage !== psSiteName)
 {
  updatedRteValue = updatedRteValue.replace(new RegExp(psSiteNameForCKEv4SavedImage, 'g'), psSiteName); }
 EditorStringObj.value = ""+updatedRteValue; }
 }
 else 
 {
 ...
 { 
 var tempname= images[i].src;   var psSiteNameForCKEv5SavedImage = images[i].getAttribute("src").split("/")[2]; var updatedRteValue = EditorStringObj.value; if(psSiteNameForCKEv5SavedImage && psSiteNameForCKEv5SavedImage !== psSiteName)
 {
  updatedRteValue = updatedRteValue.replace(new RegExp(psSiteNameForCKEv5SavedImage, 'g'), psSiteName); }
 EditorStringObj.value = ""+updatedRteValue; }
 }
 var tempval=tempname.split("###"); var ImgID='PT_RTE_IMG_DB_LOC';  var filename=tempval[1]; if(UrlID == ImgID)
 {
